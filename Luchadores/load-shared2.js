document.addEventListener('DOMContentLoaded', async () => {
  ...
  const homePath = '../inicio.html';
  const historyPath = '../historia/historia.html';
  const luchadoresPath = './luchadores.html';
  ...
  const initializeHeaderInteractions = (container) => {
    ...
    navLinks.forEach((link) => {
      ...
      const isHome = href.includes('inicio.html') || href === '#';
  ...
  if (headerTarget) {
    try {
      const response = await fetch(`${basePath}header2.html`);
  ...
  if (footerTarget) {
    try {
      const response = await fetch(`${basePath}footer2.html`);
      const html = await response.text();
